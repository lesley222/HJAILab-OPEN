...
class Decoder(nn.Module):
    def __init__(self, nf, bottle):
        ...
        self.up2 = UpSampling(nf*8 + nf*8, nf*4)
        self.up3 = UpSampling(nf*4 + nf*4, nf*2)
        self.up4 = UpSampling(nf*2 + nf*2, nf*1)
        self.up5 = UpSampling(nf*1 + nf*1, nf)

        self.out = Conv(nf+nf, 3, 3, 1, 1)
...
class Critic(nn.Module):
    def __init__(self, nc, nf=32, bottle=512):
        ...
        self.conv6 = Conv(nf*8, bottle, 4, 1, 0) # 1
...
class LocalD(nn.Module):
    def __init__(self, nc, nf=32):
        ...
        self.main = nn.Sequential(
            ConvRelu(nc, nf*1, 4, 2, 1), # 64 -> 32
            ConvSpRelu(nf*1, nf*2, 4, 2, 1), # 32 -> 16
            ConvSpRelu(nf*2, nf*4, 4, 2, 1), # 16 -> 8
