...
def warp(x, flo, padding_mode='border'):
    ...
    output = F.grid_sample(x, vgrid, padding_mode=padding_mode, mode='nearest', align_corners=False)
    return output


class TemporalLoss(nn.Module):
    def __init__(self, data_sigma=True, data_w=True, noise_level=0.001,
                 motion_level=8, shift_level=10, device=None):
    ...
    def GenerateFakeData(self, first_frame):
        ...
        if self.data_w:
            ...
            if first_frame.is_cuda:
                forward_flow = forward_flow.to(self.device)
