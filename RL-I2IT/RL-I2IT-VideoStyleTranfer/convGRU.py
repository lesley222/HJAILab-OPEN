...
class ConvGRUCell(nn.Module):
    ...
    def forward(self, input_, prev_state):
        ...
        # print(input_, prev_state)
        prev_state = prev_state.type_as(input_)
...
class ConvGRU(nn.Module):
    ...
    def forward(self, x, hidden=None):
        ...
        Parameters
        ----------
        x : 5D input tensor. (seq_len, batch, channels, height, width).
        ...
        for layer_idx in range(self.n_layers):
            ...
            for seq_idx in range(seq_len):
                cell_hidden = cell(input_[seq_idx], cell_hidden)
                next_input[seq_idx] = cell_hidden
        ...
        return next_input, torch.stack(upd_hidden, dim=0)
